...
static void buffer_init() {
	...
	for (i=0; i < (NUM_RXBUF); i++) {
		...
		rxdcpt[i]->hdr.EOWN = 1;	// Owned by HW
...
//	set_pin_strap();
	...
	IEC1CLR = _IEC1_ETHIE_MASK;	// disable Ethernet interrupts
	...
	// MAC Init
	//EMAC1MCFG=0x00008000;	// reset the MAC using SOFTRESET (EMAC1CFG1<15>)
	...
	while (readphy(PHY_ADDRESS,0) && 0x8000);// wait until reset is cleared
